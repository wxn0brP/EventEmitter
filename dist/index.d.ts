...
export declare class VEE<T extends EventMap = {}> {
     ...
     */
    on<K extends EventName<T>>(event: K, listener: T[K]): () => this;
    /**
     * Registers a one-time event listener
     * @param {K} event - event name
     * @param {Function} listener - function to be called once
     */
    once<K extends EventName<T>>(event: K, listener: T[K]): () => this;
    /**
     * Removes an event listener.
     * @param {K} event - event name
     * @param {Function} listener - listener to remove
     */
    off<K extends EventName<T>>(event: K, listener: T[K]): this;
    _emit(event: string, ...args: any[]): this;
    _matchPattern(pattern: string, event: string): boolean;
    /**
     * Emits an event
     * @param {K} event - event name
     * @param {...EventArgs<T, K>} args - arguments to be passed to listeners
     */
    emit<K extends EventName<T>>(event: K, ...args: EventArgs<T, K>): this;
    /**
